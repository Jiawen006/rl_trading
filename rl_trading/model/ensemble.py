...
from rl_trading.model import trade, train, validate
...
def ensemble_strategy(
    ...
    * sharpe_ratio : final sharpe ratio in the trading window
    ...
    for i in range(window_length, len(trade_date), window_length):
        ...
        balance_trade, shares_trade, sharpe_trade = trade.trading(
            model=trade_model,
            data=window_data,
        ...
        model_a2c = train.a2c_training(
            env_train=train_env,
            model_name=f"a2c_ensemble{i}",
            model=model_a2c,
            pretrain=False,
        )
        ...
        # start ppo training
        model_ppo = train.ppo_training(
            env_train=train_env,
            model_name=f"ppo_ensemble{i}",
            model=model_ppo,
            pretrain=False,
        )
        ...
        model_ddpg = train.ddpg_training(
            env_train=train_env,
            model_name=f"ddpg_ensemble{i}",
            model=model_ddpg,
            pretrain=False,
        )
